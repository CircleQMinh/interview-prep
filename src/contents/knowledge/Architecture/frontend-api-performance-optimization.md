...
Reduce unnecessary network round trips, but avoid creating oversized endpoints that return unrelated data. Consolidate calls when it improves latency and user experience without causing over-fetching or poor cacheability.
...
---
### Code splitting (React / front-end)
Code splitting means:

* do not send all JavaScript at once
* load only the code needed for the current page/feature
...
Debounce delays execution until rapid events stop, which is useful for search input and autocomplete. Throttle limits execution frequency, which is useful for scroll, resize, and other high-frequency events.
...
Ex: Users → Load Balancer → API Server 1 / API Server 2 / API Server 3
...
* Use async/await for I/O-bound work to improve scalability and avoid tying up request threads while waiting on external resources such as databases or HTTP calls.
...
Viewing SQL generated by EF core. It is now possible to call ToQueryString on any LINQ query, retrieving the SQL that the query would execute:
```c#
Console.WriteLine(
    ctx.Artists
...
Avoid N+1 by either eager loading related data when you truly need entities, or by projecting directly into DTOs. For API responses, projection is often the better choice because it fetches only what is needed.
...
Applying functions to indexed columns often makes predicates non-sargable, which can prevent efficient index seeks.
...
A persisted computed column is one solution. Another common and often simpler solution is rewriting the filter as a sargable date range.
...
Proper HTTP status codes help clients behave correctly and can reduce incorrect retries, but preventing retry storms also requires good retry policies, backoff, and idempotency design.
...
For large write workloads, consider batching or bulk insert/update techniques to reduce round trips and transaction overhead.
...
### Use caching when data is expensive to compute and does not change frequently:
- in-memory cache
- Redis/distributed cache
- response caching / output caching

### Pagination strategy
Offset pagination is simple but can be slow on large datasets.
Keyset/cursor pagination is often more efficient and stable for large ordered datasets.

### API compression and payload shaping
...
Middleware is typically preferred for global cross-cutting concerns because it runs early in the pipeline, applies consistently, and is framework-level rather than controller/action-specific.
